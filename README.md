...
> 💡 **提示**：
> - 推荐使用 Fedora 系统的自动化脚本，可以一键完成所有安装和配置
> - `run-auto.sh` 会自动检测您的环境（Wayland/X11、桌面环境等）
...
1. **启动程序**：
   - Fedora 系统：`./run-auto.sh`
   - 其他系统：激活虚拟环境后运行 `python src/remote_server.py`
